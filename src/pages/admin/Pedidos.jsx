...
export default function Pedidos() {
  ...
  useEffect(() => {
    const resultado = orders.filter(o => {
      ...
      const produtoOk  = filtroProduto === 'todos' || (o.items || []).some(i =>
        i.name?.toLowerCase() === filtroProduto.toLowerCase()
      )
      const dataOk     = filtroData === 'todos' || (o.pickupDate || '') === filtroData
      return mesOk && statusOk && buscaOk && produtoOk && dataOk
    })
    setPedidosFiltrados(resultado)
  }, [orders, mes, ano, filtroStatus, busca, filtroProduto, filtroData])
  ...
  const exportarCSV = () => {
    const linhas = [
      ['#', 'Data', 'Cliente', 'WhatsApp', 'Entrega', 'Data Retirada', 'Itens', 'Total', 'Status'],
      ...pedidosFiltrados.map(o => [
        o.id?.toString().slice(-6),
        o.date,
        o.customerName,
        o.customerPhone,
        ENTREGA_LABEL[o.entrega] || o.entrega,
        o.pickupDate || '',
    ...
    a.href     = url
    a.download = `pedidos-${MESES[mes]}-${ano}.csv`
    a.click()
  ...
  return (
    <div>
      ...
      <div className={styles.filterBar}>
        ...
        <select className={styles.filterSelect} value={filtroProduto} onChange={e => setFiltroProduto(e.target.value)}>
          <option value="todos">Todos os produtos</option>
          {[...new Set(products.map(p => p.name))].sort().map(name => (
            <option key={name} value={name}>{name}</option>
          ))}
        </select>
        <select className={styles.filterSelect} value={filtroData} onChange={e => setFiltroData(e.target.value)}>
          <option value="todos">Todas as datas</option>
          {[...new Set(orders.filter(o => o.pickupDate).map(o => o.pickupDate))].sort().map(data => (
        ...
        <button className={styles.exportBtn} onClick={exportarCSV} title="Exportar CSV">
          <svg width="13" height="13" viewBox="0 0 13 13" fill="none" stroke="currentColor" strokeWidth="1.8">
            <path d="M6.5 1v8M3.5 6.5l3 3 3-3"/>
            <path d="M1 10.5v1a1 1 0 001 1h9a1 1 0 001-1v-1"/>
          </svg>
          Exportar CSV
        </button>
      </div>
